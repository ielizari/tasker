...
export const jobHandlers = [
  ...
  rest.post(process.env.PUBLIC_URL + '/api/jobs/list', (req, res, ctx) => {
    const filters = req.body ? req.body as Partial<Job> : {}
    const calendars: Array<JobObject> = getTaskerRepository().getJobs(filters)

    return res(
      ctx.status(200),
      ctx.json(ApiResponseBuilder(200, calendars, false))
    )
  }),
